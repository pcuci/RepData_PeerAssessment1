...
The **mean** of the total number of steps taken per day is: `r totalMean`

The **median** of the total number of steps taken per day is: `r totalMedian`

## What is the average daily activity pattern?

A time series plot (i.e. type = "l") of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all days (y-axis)

```{r chart-activity pattern}
...
```
Interval with max steps starting at `r names(maxInterval)`
...
```
Median changed by `r medianDiff`

```{r totalcheck}
totalDiff <- sum(filledActivity$steps) - sum(activity$steps)
```

Increase of `r totalDiff` is not surprising since we filled in missing values with averages.
